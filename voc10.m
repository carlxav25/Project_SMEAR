...
voc_all = load(['/home/local/carltonx/Documents/Smear_course/voc_2010_2013.txt']);


%% Calculate median hourly VOC
id_10=find(voc_all(:,1)==2010);
voc_10=voc_all(id_10,:);

%Finding the index
[datevec_voc,~,ind]= unique(voc_10(2:end,1:4),'rows'); % find data for every hour
%Mean value of monoterpenes based on hours
mono_median_h = grpstats(voc_10(2:end,7),ind,'nanmedian'); % make medians
